...
export function canonicalizeUrl(raw: string | null): string | null {
  ...
  // Only web documents participate in lineage. A `javascript:` or `data:` URL
  // is not an article and must never be echoed back as one.
  ...
  // Sort remaining parameters so two orderings of the same query match.
...
export function outletIdFor(raw: string | null): string | null {
  ...
  try {
    ...
    // A bare domain such as "example.com" is a valid attribution value.
