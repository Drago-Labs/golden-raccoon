...
import { createRecoveryRequest, getPolicyVersion, getIncidentMode, getRecoveryConsequences } from "@/server/recovery";
...
const bodySchema = z.object({
  ...
  agent: z.enum(["execution", "decision"]).optional(),
  reason: z.string().min(1).max(280).optional(),
  contractAddress: z.string().max(128).optional(),
  consumer: z.string().max(128).optional(),
});

export async function POST(request: Request) {
  const rateLimited = checkRateLimit(request, { namespace: "recovery:revoke", limit: 10, windowMs: 60_000 });
