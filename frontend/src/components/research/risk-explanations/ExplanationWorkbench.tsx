...
function WorkbenchSession({
  ...
  // Pick up the report staged for this wallet session. A token minted for a
  // different session is refused, so a stale link cannot surface another
  // wallet's report. Results are written only from the promise continuation,
  // and the generation guard drops a response that resolves after the session
  // moved on.
  useEffect(() => {
    ...
    return () => {
      // Void any in-flight request and drop the wallet-derived report.
      generation.current += 1;
      clearStagedReports();
    };
